...
const nextConfig: NextConfig = {
  ...
  compress: true,
  // URLs WordPress legacy conservées avec trailing slash (cf. plan)
  trailingSlash: true,
  experimental: {
...
// Démarre miniflare/workerd uniquement à la demande explicite (CF_DEV=1),
// sinon `next dev` lance un sous-process Workers qui double l'empreinte mémoire.
// → dev rapide : `npm run dev`
// → dev avec bindings Cloudflare (KV, R2, secrets…) : `npm run dev:cf`
